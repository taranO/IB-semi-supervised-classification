...
parser.add_argument("--dense_filters", default=[1024,500], help="")
parser.add_argument("--is_bn",         default=False, type=int, help="...") # 2e-4

parser.add_argument("--lr",           default=1e-3, type=float, help="Training learning rate") # 2e-4
parser.add_argument("--weight_decay", default=1e-3, type=float, help="Training weight decay") # 6e-8
...
parser.add_argument("--alpha",          default=0, type=float, help="Training learning rate")
parser.add_argument("--checkpoint_dir", default="model_BIB_Dc_%dconv_3dense_%sbn_%s_%s_%snoise_%0.2f_run_%s",   type=str, help="Dir where to same models")
...
if __name__ == "__main__":
    for run in range(1, args.runs + 1):
        ...
        checkpoint_dir = args.checkpoint_dir % (len(args.conv_filters),
                                                     "" if args.is_bn else "no_",
        ...
        # --- print log --------------------------------------------------------
        info = {"checkpoint_dir_": checkpoint_dir_, "results_dir_": results_dir_,
                '#_samples': "all" if args.supervised_n == 0 else args.supervised_n,
                '#_classes': args.classes, "conv_filters": args.conv_filters, 'dense_filters': args.dense_filters, "is_bn": args.is_bn, "alpha": args.alpha,
